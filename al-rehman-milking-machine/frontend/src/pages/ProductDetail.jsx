    ...
    <div className="max-w-6xl mx-auto px-5 py-14">
      <div className="grid md:grid-cols-2 gap-10 items-start">
              ...
              {product.images.slice(0, 5).map((img, i) => (
                <motion.button
                  ...
                  className={`aspect-square rounded-lg overflow-hidden border transition-all ${
                    activeImage === img ? "border-brand-600 ring-2 ring-brand-500/20" : "border-gray-200"
                  }`}
                >
        ...
        >
          ...
          {/* Description */}
          <p className="text-gray-600 leading-relaxed mb-7 whitespace-pre-line">{product.description}</p>

          {/* Specifications */}
          {(spec.capacity || spec.motorPower || spec.material || spec.warranty) && (
            <motion.div
              ...
              <div className="grid grid-cols-2 gap-5 text-sm">
                {spec.capacity && (
                  <div>
                    ...
                    <p className="font-medium text-ink-950">{spec.capacity}</p>
                  </div>
                )}
                ...
                {spec.motorPower && (
                  <div>
                    ...
                    <p className="font-medium text-ink-950">{spec.motorPower}</p>
                  </div>
                )}
                ...
                {spec.material && (
                  <div>
                    ...
                    <p className="font-medium text-ink-950">{spec.material}</p>
                  </div>
                )}
                ...
                {spec.warranty && (
                  <div>
                    ...
                    <p className="font-medium text-ink-950">{spec.warranty}</p>
                  </div>
                )}
              </div>
            </motion.div>
          ...
          <div className="flex items-center gap-5 mb-6">
            <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden">
              ...
              <span className="px-5 py-2 border-x border-gray-200 font-semibold">{qty}</span>
              <button onClick={() => setQty((q) => q + 1)} className="px-4 py-2 hover:bg-gray-100 transition">
                +
              </button>
            </div>
          </div>
